...
import IPCsdk.crypto.DigestHash;
...
public class Tools {
     ...
     * @return
     */
    public static String byteToString(int[] version, byte[] input) {
        if(input == null || input.length == 0){
            throw new IllegalArgumentException("传入的字符串为空！");
        }
        ...
        byte[] address = new byte[3 + length];
        // 加入version。
        address[0] = (byte)version[0];
        address[1] = (byte)version[1];
        address[2] = (byte)version[2];
        // 加入pubKeyHash部分，此时数组中的数据为keyHash（version + pubKeyHash）部分。
        System.arraycopy(input, 0, address, 3, length);
    ...
    public static String pubKeyHexToAddress(String pubKeyHex, NetworkParameters params) {
        ...
        Address address = new LegacyAddress(params, DigestHash.hash160(pubKey));
    ...
    public static String ecKeyToAddress(ECKey ecKey, NetworkParameters params){
        ...
        return Tools.byteToString( params.getAddressHeader(), pubKey);
    }
